...
<div class="portlet box blue">
  ...
  for($i=0;$i<count($val);$i++){
    ...
    if ($sqll>0) {
      if ($sqll1['courier_payment']=='due' && $sqll1['courier']!='speed post') {
        $query = "UPDATE `customorder` SET `courier_payment`='done',`courier_payment_date`='".$date."' WHERE `customorder_id`='".$order_id."'";  
          mysqli_query($con, $query);
          //$mobile = $sqll1['mobile'];
        
          //send_sms_for_order($order_id,$mobile,$barcode);
      }else{
           if($sqll1['courier']=='speed post'){
              $order_status='speed post';
           }else{
               $order_status='already payment done';
           }
