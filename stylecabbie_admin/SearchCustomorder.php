  ...
  <div class="container"> <!-- Start : Inner Page container -->
    <div class="crumbs">    <!-- Start : Breadcrumbs -->
        <ul id="breadcrumbs" class="breadcrumb">
            ...
            <li class="current">Custom Order Detail</li>
        </ul>
    </div>  <!-- End : Breadcrumbs -->
        ...
        <form  method="GET" action="" enctype="multipart/form-data">
              <div class="form-group has-success col-md-4">
                  <select class="form-control" name="searchType">
                          <option <?php if($_GET['searchType']=='order_id'){ echo'selected'; } ?> value="order_id">Order Id</option>
                          <option <?php if($_GET['searchType']=='mobile_no'){ echo'selected'; } ?> value="mobile_no">Mobile No</option>
                          <option <?php if($_GET['searchType']=='name'){ echo'selected'; } ?> value="name">Name</option>
                 </select>
              </div>
              ...
              <tbody>
                ...
                if($_GET['btn_search']!=''){
                    ...
                    if($searchType=='order_id'){
                        $query="select * from `customorder` where customorder_id='$mobile' ORDER BY customorder_id DESC";
                    }
                    if($searchType=='mobile_no'){
                        $query="select * from `customorder` where mobile='$mobile' ORDER BY customorder_id DESC";
                    }
                    if($searchType=='name'){
                      $mobile='%'.$mobile.'%';
                      $query="select * from `customorder`  WHERE name LIKE '$mobile' ORDER BY customorder_id DESC";
                    }
                        ...
                        <select class="form-control" name="status" onChange="changeStatus(this.value,<?php echo $row_con['customorder_id'];?>);">
                          ...
                          <option <?php if($row_con['status']=='1'){ echo "selected"; } ?> value="1"><?php echo 'Confirm';?></option>
                           <option <?php if($row_con['status']=='8'){ echo "selected"; } ?> value="8">Return</option>
                            <option <?php if($row_con['status']=='23'){ echo "selected"; } ?> value="23"><?php echo 'Ready to print';?></option>
