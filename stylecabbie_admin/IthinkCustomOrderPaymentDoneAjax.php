...
<div class="portlet box blue">
  ...
  for($i=0;$i<count($val);$i++){
    ...
    if ($sqll>0) {
      if ($sqll1['cod_payment_received']=='21') {
        $query = "UPDATE `customorder` SET `cod_payment_received`='done',`update_at`='$date' WHERE `customorder_id`='".$order_id."'";  
          mysqli_query($con, $query);
          //$mobile = $sqll1['mobile'];
        
          //send_sms_for_order($order_id,$mobile,$barcode);
      }else{
        $order_status='already done';
      }
