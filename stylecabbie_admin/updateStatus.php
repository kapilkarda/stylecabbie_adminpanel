...
function send_sms_for_order($order_id,$status,$mobile){
			...
			if($status=='9' || $status=='1'){
			  	...
			  	$msg="Your Order (ORDER ID:".$order_id.") has been dispatched. Check your order history for order tracking.";
...
if(isset($_GET['status']) && ($_GET['oid']))
{
	...
	if($_GET['method']=='CustomOrder'){
		   ...
		   $sql = mysqli_query($con,"update `customorder` set status='$status',update_at='$date' where customorder_id='$oid'");
		    if ($sql) {
		    	echo 1;
